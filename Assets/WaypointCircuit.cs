    ...
    void CachePositionsAndDistances()
    {
        ...
        for (int i = 0; i < points.Length; ++i)
        {
            ...
            if (t1 != null && t2 != null)
            {
                ...
                //Debug.LogFormat("--- wp{0}", Waypoints[(i) % Waypoints.Length]);
                // Debug.LogFormat("--- 2wp{0}", Waypoints[(i+1) % Waypoints.Length]);
        ...
        if (Waypoints.Length > 1)
        {
            ...
            if (smoothRoute)
            {
                ...
                for (float dist = 0; dist < Length; dist += Length / editorVisualisationSubsteps)
                {
                    ...
                    prev = next;
                    //Debug.LogFormat("--- dis{0}", dist);
                }
                Gizmos.DrawLine(Waypoints[10].position, Waypoints[11].position);
            }
            else
            {
...
public class WaypointListDrawer : PropertyDrawer
{
    ...
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        if (items.arraySize > 0)
        {
            ...
            for (int i = 0; i < items.arraySize; ++i)//for (int i = -1; i < items.arraySize; ++i)
            {
